...
static struct bbuf __bounce_alloc(struct bch_fs *c, unsigned size, int rw)
{
	...
	b = kmalloc(size, GFP_NOFS|__GFP_NOWARN|__GFP_RECLAIMABLE|__GFP_NORETRY);
	if (b)
		return (struct bbuf) { .c = c, .b = b, .type = BB_kmalloc, .rw = rw };

	b = mempool_alloc(&c->compress.bounce[rw], GFP_NOFS|__GFP_RECLAIMABLE|__GFP_NORETRY);
	if (b)
				       ...
				       struct bvec_iter start, int rw)
{
	...
	struct page **pages = nr_pages > ARRAY_SIZE(stack_pages)
		? kmalloc_array(nr_pages, sizeof(struct page *), GFP_NOFS|__GFP_RECLAIMABLE|__GFP_NORETRY)
		: stack_pages;
	if (!pages)
		return bio_bounce(c, bio, start, rw);
			  ...
			  struct bch_extent_crc_unpacked crc)
{
	...
	case BCH_COMPRESSION_TYPE_gzip: {
		...
		void *workspace = mempool_alloc(workspace_pool, GFP_NOFS|__GFP_RECLAIMABLE|__GFP_NORETRY);
