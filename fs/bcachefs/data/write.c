			       ...
			       bool nocow, struct bch_dev **cas)
{
	...
	bkey_for_each_ptr(ptrs, ptr) {
		...
		if (ptr != last) {
			n = to_wbio(bio_alloc_clone(NULL, &wbio->bio, GFP_NOFS|__GFP_RECLAIMABLE, &c->replica_set));
					...
					void *buf)
{
	...
	bio = bio_alloc_bioset(NULL, pages, 0, GFP_NOFS|__GFP_RECLAIMABLE, &c->bio_write);
	wbio			= wbio_init(bio);
	wbio->put_bio		= true;
			     ...
			     struct bio **_dst)
{
	...
	if (dst == src && more) {
		...
		dst = bio_split(src, total_input >> 9,
				GFP_NOFS|__GFP_RECLAIMABLE, &c->bio_write);
