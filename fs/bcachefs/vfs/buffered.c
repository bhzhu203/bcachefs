...
int bch2_writepages(struct address_space *mapping, struct writeback_control *wbc)
{
	...
	struct bch_writepage_state *w = kzalloc(sizeof(*w), GFP_NOFS|__GFP_NOFAIL|__GFP_RECLAIMABLE);

	w->tmp = mempool_alloc(&c->vfs.writepage_buf_pool, GFP_NOFS|__GFP_RECLAIMABLE);
